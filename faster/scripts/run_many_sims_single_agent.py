...
if __name__ == '__main__':
    ...
    for i in range(num_of_sims):
        ...
        commands.append("11");
        commands.append("sleep 1.5 && 22");
        commands.append("sleep 1.5 && cd "+folder_bags+" && rosbag record -o "+name_bag+"_"+str(i)+" /SQ01s/goal __name:="+name_node_record);
        #publishing the goal should be the last command
        commands.append("sleep 1.5 && rostopic pub /SQ01s/term_goal geometry_msgs/PoseStamped \'{header: {stamp: now, frame_id: \"world\"}, pose: {position: {x: 50, y: 0, z: 1}, orientation: {w: 1.0}}}\'");
        ...
        os.system("tmux kill-server & killall -9 gazebo & killall -9 gzserver  & killall -9 gzclient & killall -9 roscore & killall -9 rosmaster & pkill faster_node & pkill -f dynamic_obstacles & pkill -f rosout & pkill -f behavior_selector_node & pkill -f rviz & pkill -f rqt_gui & pkill -f perfect_tracker & pkill -f faster_commands")
