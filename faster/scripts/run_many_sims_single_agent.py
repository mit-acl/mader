    ...
    for k in range(len(total_num_of_obs)):
        for j in range(len(all_basis)):
            ...
            for s in range(num_of_sims):
                ...
                commands.append("roslaunch faster all.launch gui:=false rviz:=false environment:=false");
                commands.append("sleep 1.0 &&rosrun faster dynamic_obstacles.py "+str(total_num_of_obs[k]));
