...
class GoalReachedCheck:
    ...
    def SQ01term_goalCB(self, data):
        self.term_goal_pos[0,0:3] = np.array([data.pos.x, data.pos.y, data.pos.z])
        self.initialized = True
    def SQ02term_goalCB(self, data):
        self.term_goal_pos[1,0:3] = np.array([data.pos.x, data.pos.y, data.pos.z])
    def SQ03term_goalCB(self, data):
        self.term_goal_pos[2,0:3] = np.array([data.pos.x, data.pos.y, data.pos.z])
    def SQ04term_goalCB(self, data):
        self.term_goal_pos[3,0:3] = np.array([data.pos.x, data.pos.y, data.pos.z])
    def SQ05term_goalCB(self, data):
        self.term_goal_pos[4,0:3] = np.array([data.pos.x, data.pos.y, data.pos.z])
    def SQ06term_goalCB(self, data):
        self.term_goal_pos[5,0:3] = np.array([data.pos.x, data.pos.y, data.pos.z])
    def SQ07term_goalCB(self, data):
        self.term_goal_pos[6,0:3] = np.array([data.pos.x, data.pos.y, data.pos.z])
    def SQ08term_goalCB(self, data):
        self.term_goal_pos[7,0:3] = np.array([data.pos.x, data.pos.y, data.pos.z])
    def SQ09term_goalCB(self, data):
        self.term_goal_pos[8,0:3] = np.array([data.pos.x, data.pos.y, data.pos.z])
    def SQ10term_goalCB(self, data):
        self.term_goal_pos[9,0:3] = np.array([data.pos.x, data.pos.y, data.pos.z])
...
def startNode():
    ...
    rospy.Subscriber("SQ01s/term_goal", State, c.SQ01term_goalCB)
    rospy.Subscriber("SQ02s/term_goal", State, c.SQ02term_goalCB)
    rospy.Subscriber("SQ03s/term_goal", State, c.SQ03term_goalCB)
    rospy.Subscriber("SQ04s/term_goal", State, c.SQ04term_goalCB)
    rospy.Subscriber("SQ05s/term_goal", State, c.SQ05term_goalCB)
    rospy.Subscriber("SQ06s/term_goal", State, c.SQ06term_goalCB)
    rospy.Subscriber("SQ07s/term_goal", State, c.SQ07term_goalCB)
    rospy.Subscriber("SQ08s/term_goal", State, c.SQ08term_goalCB)
    rospy.Subscriber("SQ09s/term_goal", State, c.SQ09term_goalCB)
    rospy.Subscriber("SQ10s/term_goal", State, c.SQ10term_goalCB)
