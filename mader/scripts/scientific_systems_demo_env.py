...
# To avoid helipad created three different types of obstacles
class MovingForest1:
    def __init__(self, total_num_obs):
        ...
        self.y_min= -2.35
        self.y_max= 0.0
        self.z_min= 0.0 #-6.0 for sphere sim
        self.z_max= 5.0  #6.0 for sphere sim
        self.scale=1.0;
        self.slower_min=1.2
        self.slower_max= 1.2
...
class FakeSim:
    ...
    def __init__(self, total_num_obs):
        ...
        tno1 = round(2.35*(20-2.35)/(20*11.25)*total_num_obs)
        tno2 = round(2.35*(11.25-2.35)/(20*11.25)*total_num_obs)
        tno3 = round((11.25-2.35)*(20-2.35)/(20*11.25)*total_num_obs)

        self.world1=MovingForest1(tno1)
        self.world2=MovingForest2(tno2)
        self.world3=MovingForest3(tno3)
        ...
        for self.world in [self.world1, self.world2, self.world3]:
            for i in range(self.world.num_of_dyn_objects):          
                self.x_all.append(random.uniform(self.world.x_min+3, self.world.x_max-3));
                self.y_all.append(random.uniform(self.world.y_min+3, self.world.y_max-3));
        ...
        self.pubDemoSpace = rospy.Publisher('/demo_space', Marker, queue_size=1, latch=True)
        self.pubHelipad = rospy.Publisher('/helipad', Marker, queue_size=1, latch=True)

        self.already_published_static_shapes=False;

        rospy.sleep(0.5)

    def demo_space(self, timer):
        ...
        self.pubDemoSpace.publish(marker)

    def helipad(self, timer):
        ...
        y_max = 0.0
        y_min = -2.35
        z_max = 1.0 #in the above url it's 2 but it's just for safety
        z_min = 0.0
        marker.scale.x = x_max - x_min
        marker.scale.y = y_max - y_min
        marker.scale.z = z_max - z_min
        ...
        # marker.lifetime = rospy.Duration(0.3)
        marker.lifetime = rospy.Duration.from_sec(0.0);
        self.pubHelipad.publish(marker)
...
def startNode(total_num_obs):
    ...
    rospy.Timer(rospy.Duration(0.01), c.demo_space)
    rospy.Timer(rospy.Duration(0.01), c.helipad)
