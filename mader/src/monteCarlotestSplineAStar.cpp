...
int main(int argc, char **argv)
{
  ...
  for (int novale = 0; novale < 200; novale++)
  {
    std::cout << "Creating " << num_of_obstacles << " obstacles" << std::endl;
    ...
    double runtime = 3.0;     //[seconds]
    double goal_size = 0.01;  //[meters]

    Eigen::Vector3d v_max(7.0, 7.0, 7.0);
    Eigen::Vector3d a_max(400000.0, 4000000.0, 4000000.0);

    double min_world = -3.0;
    double max_world = 3.0;

    Eigen::Vector3d q0(min_world - 5, 0, 0);
    Eigen::Vector3d q1 = q0;
    Eigen::Vector3d q2 = q1;
    Eigen::Vector3d goal(max_world + 5, 0, 0);

    double t_min = 0.0;
    double t_max = t_min + (goal - q0).norm() / (0.2 * v_max(0));
    ...
    std::uniform_real_distribution<double> distribution(min_world, max_world);

    for (int j = 0; j < num_of_obstacles; j++)
    {
      double random_x = distribution(gen);
      double random_y = distribution(gen);

      ConvexHullsOfCurve hulls_curve = createStaticObstacle(random_x, random_y, 0.0, num_pol, bbox_x, bbox_y, bbox_z);

      std::cout << "Obst at " << random_x << ", " << random_y << ", " << 0.0 << std::endl;
    ...
    myAStarSolver.setBBoxSearch(300.0, 300.0, 300.0);  // limits for the search, centered on q2
    ...
    marker_array_best_traj = trajectory2ColoredMarkerArray(best_traj_found, type, v_max.maxCoeff(), increm,
    ...
    // ros::spinOnce();
    double cost = myAStarSolver.getCost();
