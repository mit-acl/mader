...
if __name__ == '__main__':
    ...
    for dc in dc_list:
        ...
        for i in range(len(csv)):
            ...
            np.add(dist_matrix_dc, dist_matrix)

        # averageing over num of sim
        num_of_sims_matrix = num_of_sims * np.ones((num_of_sims,num_of_sims))
        ...
        for i in range(num_of_agents):
            os.system('echo "SQ '+i+' '+ave_per_dc[i]+'" >> '+source_dir+'/ave_distance.txt')

        total_ave_dist = sum(ave_per_dc)/len(ave_per_dc)
        os.system('echo "total distance '+total_ave_dist+'" >> '+source_dir+'/ave_distance.txt')
